...
class CMOSMetalBayerFilter3D(device.Device):

	def __init__(self, size, permittivity_bounds, init_permittivity, num_z_layers):
		super(CMOSMetalBayerFilter3D, self).__init__(size, permittivity_bounds, init_permittivity)
	...
	def update_filters(self, epoch):
		...
		self.layering_y_0 = layering.Layering(self.y_dimension_idx, self.num_z_layers)
	...
	def init_filters_and_variables(self):
		...
		# self.filters = [self.layering_y_0, self.sigmoid_1, self.scale_2]#[self.layering_y_0, self.scale_1]# [self.sigmoid_0, self.layering_z_1, self.max_blur_xy_2, self.sigmoid_3, self.scale_4]
		self.filters = [self.layering_z_0, self.scale_1]#[self.layering_y_0, self.scale_1]# [self.sigmoid_0, self.layering_z_1, self.max_blur_xy_2, self.sigmoid_3, self.scale_4]
		# self.filters = [self.scale_1]
