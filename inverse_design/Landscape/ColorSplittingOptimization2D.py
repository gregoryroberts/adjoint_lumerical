...
class ColorSplittingOptimization2D():
	...
	def optimize(
		...
		for iter_idx in range( 0, num_iterations ):
			...
			cur_fab_penalty = self.compute_fab_penalty_fourier( self.design_density, fourier_fab_penalty_feature_size )
			...
			if index_contrast_regularization:
				...
				# sqrt( ( sum() )**2 )--> 2 * sum() * d/d(internal)

				grad_index_reg = -( 
					( net_gradient_index_contrast - net_gradient ) / index_contrast_df_h ) / index_reg_denominator
