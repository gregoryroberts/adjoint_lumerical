...
class ColorSplittingOptimizationALD2D():
	...
	def __init__( self,
		...
		self.high_ald_index = 2.5
		self.high_ald_permittivity = self.high_ald_index**2
		self.delta_ald_permittivity = self.high_ald_permittivity - self.min_relative_permittivity
	...
	def interpret_ald( self, density, gradient ):
		...
		term_2 = ( 1 - density ) * self.delta_ald_permittivity * (
			high_index_grad_mask_horizontal * ( pad_gradient[ 0:(density.shape[0] - 1), 1:(density.shape[1]) ] - pad_gradient[ 2:, 1:(density.shape[1]) ] ) +
			high_index_grad_mask_vertical * ( pad_gradient[ 1:(density.shape[0]), 0:(density.shape[1] - 1) ] - pad_gradient[ 1:(density.shape[0]), 2: ] )
		)
	...
	def optimize(
		...
		for iter_idx in range( 0, num_iterations ):
			...
			net_gradient = self.interpret_ald( import_density, net_gradient )
