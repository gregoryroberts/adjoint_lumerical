...
class ColorSplittingOptimization2DSigmoid():
	...
	def density_to_permittivity( self, density, epoch ):
		density_sigmoid = sigmoid.Sigmoid( 0.125 * ( 2**epoch ), 0.5 )
		forward_density = density_sigmoid.forward( density )
		return ( self.min_relative_permittivity + ( self.max_relative_permittivity - self.min_relative_permittivity ) * forward_density )

	def apply_sigmoid( self, gradient, density, epoch ):
		density_sigmoid = sigmoid.Sigmoid( 0.125 * ( 2**epoch ), 0.5 )
		forward_density = density_sigmoid.forward( density )

		return density_sigmoid.chain_rule( gradient, forward_density, density )
	...
	def optimize(
		...
		for iter_idx in range( 0, num_iterations ):
			...
			#
			import_density = upsample( self.design_density, self.coarsen_factor )
			net_gradient = self.apply_sigmoid( net_gradient, import_density, epoch )
