...
import LayeredLithographyIRPolarizationRotationDevice
...
def figure_of_merit( Qxx, Qxy, Qyx, Qyy ):
	...
	for focal_spot_idx in range( 0, num_focal_spots ):
		...
		Qxx_focal_spot = Qxx[ focal_spot_idx, : ] / np.sqrt( max_intensity_by_wavelength_by_focal_point[ focal_spot_idx ] )
		Qxy_focal_spot = Qxy[ focal_spot_idx, : ] / np.sqrt( max_intensity_by_wavelength_by_focal_point[ focal_spot_idx ] )
		Qyx_focal_spot = Qyx[ focal_spot_idx, : ] / np.sqrt( max_intensity_by_wavelength_by_focal_point[ focal_spot_idx ] )
		Qyy_focal_spot = Qyy[ focal_spot_idx, : ] / np.sqrt( max_intensity_by_wavelength_by_focal_point[ focal_spot_idx ] )
...
def gradient(
	...
	fom_weightings = np.zeros( rearrange_figures_of_merit.shape )

	exponential_weightings = False
	if exponential_weightings:
...
shutil.copy2(python_src_directory + "/LayeredLithographyIRPolarizationRotationParameters.py", projects_directory_location + "/LayeredLithographyIRPolarizationRotationParameters.py")
...
bayer_filter = LayeredLithographyIRPolarizationRotationDevice.LayeredLithographyIRPolarizationRotationDevice(
...
for epoch in range(start_epoch, num_epochs):
	...
	for iteration in range(0, num_iterations_per_epoch):
		...
		for focal_idx in range( 0, num_focal_spots ):
			...
			parallel_intensity_per_focal_spot_evolution[ epoch, iteration, focal_idx, : ] = parallel_intensity / max_intensity_by_wavelength_by_focal_point[ focal_idx ]
			orthogonal_intensity_per_focal_spot_evolution[ epoch, iteration, focal_idx, : ] = orthogonal_intensity / max_intensity_by_wavelength_by_focal_point[ focal_idx ]
