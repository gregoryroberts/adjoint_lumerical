...
from CMOSDielectricBayerFilterParameters import *

class CMOSDielectricBayerFilter(device.Device):

	def __init__(self, size, permittivity_bounds, init_permittivity, num_z_layers):
		super(CMOSDielectricBayerFilter, self).__init__(size, permittivity_bounds, init_permittivity)
	...
	def update_permittivity(self):
		...
		scale_real_4 = self.scale_4[ 0 ]
		scale_imag_4 = self.scale_4[ 1 ]

		var5 = scale_real_4.forward( var4 ) + 1j * scale_imag_4.forward( var4 )
		self.w[5] = var5
	...
	def backpropagate(self, gradient_real, gradient_imag):
		...
		gradient = (
			scale_real_4.chain_rule( gradient_real, self.w[5], self.w[4] ) +
			scale_imag_4.chain_rule( gradient_imag, self.w[5], self.w[4] )
		)	
