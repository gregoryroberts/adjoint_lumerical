...
projects_directory_location = projects_directory_location_base + '_narrow_angular_bfast_32_snell_large_focal_xpol_v3'#_dilated_250nm'
projects_directory_location2 = projects_directory_location_base + '_narrow_angular_bfast_32_snell_large_focal_ypol_v3'#_dilated_250nm'


angular_focal_fields = np.load( projects_directory_location + "/angular_focal_fields.npy" )
angular_focal_fields2 = np.load( projects_directory_location + "/angular_focal_fields.npy" )

angular_focal_fields_arrays = [ angular_focal_fields, angular_focal_fields2 ]
...
for wl_idx in range( 0, num_design_frequency_points ):
	...
	coherent_fields = np.zeros( ( 3, 1 + fdtd_region_minimum_lateral_voxels, 1 + fdtd_region_minimum_lateral_voxels ), dtype=np.complex )

	for field_idx in range( 0, len( angular_focal_fields_arrays ) ):
		fields_by_wl = np.squeeze( angular_focal_fields_arrays[ field_idx ][ :, :, :, :, :, wl_idx ] )
...
for wl_idx in range( 0, num_design_frequency_points ):
	...
	incoherent_fields = np.zeros( ( 3, 1 + fdtd_region_minimum_lateral_voxels, 1 + fdtd_region_minimum_lateral_voxels ), dtype=np.complex )

	averaged_incoherent_intensity = np.zeros( ( 1 + fdtd_region_minimum_lateral_voxels, 1 + fdtd_region_minimum_lateral_voxels ) )
	...
	for avg_idx in range( 0, num_incoherent_sums ):
		...
		averaged_incoherent_intensity += 0.5 * ( 1. / num_incoherent_sums ) * np.squeeze( np.sum( np.abs( averaged_incoherent_fields )**2, axis=0 ) )
