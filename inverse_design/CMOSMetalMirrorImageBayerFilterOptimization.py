...
from CMOSMetalMirrorImageBayerFilterParameters import *
...
design_imports_mirror = []
bayer_filter_regions_mirror_z = []
...
for device_layer_idx in range( 0, number_device_layers ):
	...
	if is_layer_designable[ device_layer_idx ]:
		...
		layer_import_mirror = fdtd_hook.addimport()
		layer_import_mirror['name'] = 'layer_import_mirror_' + str( device_layer_idx )
		layer_import_mirror['x span'] = device_size_lateral_um * 1e-6
		layer_import_mirror['y span'] = device_size_lateral_um * 1e-6
		layer_import_mirror['z min'] = -layer_vertical_maximum_um * 1e-6
		layer_import_mirror['z max'] = -layer_vertical_minimum_um * 1e-6

		bayer_filter_region_mirror_z = 1e-6 * np.linspace(-layer_vertical_maximum_um, -layer_vertical_minimum_um, layer_thicknesses_voxels[device_layer_idx])
...
def import_bayer_filters():
	...
	for device_layer_idx in range( 0, len( bayer_filters ) ):
		...
		design_import_mirror = design_imports_mirror[ device_layer_idx ]

		fdtd_hook.select( design_import_mirror[ "name" ] )
		fdtd_hook.importnk2( np.flip( cur_index, axis=2 ), bayer_filter_region_x, bayer_filter_region_y, bayer_filter_regions_mirror_z[ device_layer_idx ] )
...
for epoch in range(start_epoch, num_epochs):
	...
	for layer_idx in range( 0, halfway_point ):
		...
		#
		gradient_shape = device_gradient_real.shape
		...
		update_bayer_filters( -average_real_mirror_gradient, -average_imag_mirror_gradient, step_size )
