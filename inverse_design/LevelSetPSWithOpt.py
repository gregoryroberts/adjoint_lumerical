...
projects_directory_location += "/" + project_name + "_particle_swarm_Hz_sio2_v5"
...
shutil.copy2(python_src_directory + "/LevelSetPSWithOpt.py", projects_directory_location + "/LevelSetPSWithOpt.py")
...
for epoch_idx in range( 0, num_epochs ):
	if last_swarm:
		my_optimization_state, local_fom = optimize_parent_locally( my_optimization_state, num_local_cycles )

		for fom_idx in range( 0, len( local_fom ) ):
			figure_of_merit_evolution.append( local_fom[ fom_idx ] )

		if num_local_cycles > 0:
			log_file = open( projects_directory_location + "/log.txt", 'a' )
			log_file.write( str( local_fom ) + "\n" )
			log_file.close()

	random_profile_idx = np.minimum(
		int( np.random.random() * len( device_layer_thicknesses_um ) ),
		len( device_layer_thicknesses_um ) - 1 )

	last_swarm, init_swarm_foms, final_swarm_foms = my_optimization_state.run_swarm( random_profile_idx, num_particles, num_swarm_iterations, evaluate_device )

	log_file = open( projects_directory_location + "/log.txt", 'a' )
	log_file.write( "Random profile index = " + str( random_profile_idx ) + "\n" )
	log_file.write( "Swarm success = " + str( last_swarm ) + "\n" )
	log_file.close()

	np.save( projects_directory_location + "/init_swarm_foms_" + str( epoch_idx ) + ".npy", init_swarm_foms )
	np.save( projects_directory_location + "/final_swarm_foms_" + str( epoch_idx ) + ".npy", final_swarm_foms )
...
np.save( projects_directory_location + "/final_swarm_foms_by_epoch.npy", final_swarm_foms_by_epoch )
